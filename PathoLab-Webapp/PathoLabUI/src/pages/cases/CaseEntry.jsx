...
const CaseEntry = () => {
    ...
    const [caseData, setCaseData] = useState({
        referral: location.state?.referral || 'Self',
        sampleType: location.state?.sampleType || 'Blood',
    ...
    useEffect(() => {
        const loadInitialData = async () => {
            try {
                const [patientsData, testsData, doctorsData] = await Promise.all([
                    api.getPatients(),
                    api.getTests(),
                    api.getDoctors()
                ]);
    ...
    const handleCreateCase = async () => {
        try {
            ...
            const casePayload = {
                id: caseIdFromState || `CASE-${Date.now().toString().slice(-6)}`,
                ...
                tests: selectedTests.map(t => ({
                    id: t.id,
                    testName: t.testName,
                    // Preserve value if it exists (from location.state)
            ...
            if (caseIdFromState) {
                ...
                const { id, ...updateData } = casePayload;
                await api.updateCase(caseIdFromState, updateData);
                navigate(`/case-details/${caseIdFromState}`);
            } else {
                const savedCase = await api.createCase(casePayload);
        ...
        <div className="p-10 mx-auto space-y-8">
            ...
            <div className={`bg-white border border-gray-200 rounded-xl p-8 shadow-sm transition-all duration-300 ${selectedPatient ? 'opacity-100' : 'opacity-40 grayscale pointer-events-none'}`}>
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="relative">
                            ...
                            <input
                                ...
                                onFocus={() => setShowAllTests(true)}
                            />
                            {(searchQuery || showAllTests) && (
                                <>
                                    <div
                                            ...
                                            filteredTests.map(test => (
                                                ...
                                                >
                                                    <div className="flex flex-col">
                                                        <span className="text-sm font-bold text-gray-800">{test.testName}</span>
                                                        <span className="text-[10px] text-gray-400 font-bold uppercase tracking-tighter">{test.department}</span>
                                                    </div>
                                                    <Plus className="w-4 h-4 text-blue-500" />
                                                </button>
                                            ))
                                        ) : (
                                            <div className="px-4 py-3 text-center text-xs text-gray-400 italic">No tests found</div>
                                        )}
                                    </div>
                                </>
                            )}
                ...
                <div className="mt-10 pt-6 border-t flex justify-end">
                    ...
                    >
                        {caseIdFromState ? 'Update Case & Enter Results' : 'Create Case & Enter Results'}
